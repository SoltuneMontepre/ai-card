...
import { analyzeStep4, type StepContext } from "@/lib/gemini";
import {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await analyzeStep4(text, getRateLimitKeyFromRequest(request), context);
    ...
    console.error("[verify/step4]", message);

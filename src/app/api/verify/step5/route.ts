...
import { analyzeStep5, type StepContext } from "@/lib/gemini";
import {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await analyzeStep5(text, getRateLimitKeyFromRequest(request), context);
    ...
    console.error("[verify/step5]", message);
